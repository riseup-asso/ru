...
namespace Drupal\media_gallery_migration2\Plugin\migrate\source;
 ...
 *
 * @MigrateSource(
 *   id = "d7_media_gallery_file",
 *   source_module = "file"
 * )
 */
class MediaGalleryFile extends DrupalSqlBase {
  ...
  public function prepareRow(Row $row) {
    ...
    $media_gallery_fids = $this->select('field_data_media_gallery_file', 'm')
      ->fields('m', ['media_gallery_file_fid'])
      ->condition('media_gallery_file_fid', $fid)
      ->execute()
      ->fetch();

    // If file not in gallery, skip it.
    if ($media_gallery_fids === FALSE) {
      return FALSE;
    }

    $alt = $this->select('field_data_media_title', 'm')
      ->fields('m', ['media_title_value'])
    ...
    if ($alt) {
      ...
      $row->setSourceProperty('alt', 'media image');
    }
